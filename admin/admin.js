(() => {
  ...
  const PJ_GENRES=['시','서간','잡저','기문','축문','제문','애사','묘지명','행장','복합·경계','기타'];
  ...
  const pjTitle=()=>`평재문집 제${Number($('#pj-book').value)||1}책 제${Number($('#pj-sheet').value)||1}장 앞·뒷면`;
  const pjBookSheetCompare=(a,b)=>(Number(a.book_no)-Number(b.book_no))||(Number(a.sheet_no??a.start_page)-Number(b.sheet_no??b.start_page));
  ...
  function pjUpdateTitle(){if($('#pj-title'))$('#pj-title').value=pjTitle();const sheet=Number($('#pj-sheet').value)||1;pjPages.forEach(p=>p.page=sheet);pjDuplicateWarning()}
  function pjRenderPage(){const box=$('#pj-page-editor'),sheet=Number($('#pj-sheet').value)||1,page=pjPages[pjPageIndex]||pjBlankPage(pjPageIndex?'back':'front',sheet);$('#pj-page-tabs').innerHTML=pjPages.map((p,i)=>`<button class="pj-page-tab ${i===pjPageIndex?'active':''}" type="button" data-pj-page="${i}">${p.side==='back'?'뒷면':'앞면'}</button>`).join('');$('#pj-page-editor').innerHTML=`<div class="pj-page-meta"><strong>제${sheet}장 ${page.side==='back'?'뒷면':'앞면'}</strong><label>면 안의 글 종류<select data-pj-field="genre">${PJ_GENRES.map(x=>`<option ${x===page.genre?'selected':''}>${x}</option>`).join('')}</select></label><label>작품명<input data-pj-field="work_title" value="${escapeText(page.work_title)}"></label><label>이어짐·판독 표식<input data-pj-field="marker" value="${escapeText(page.marker)}" placeholder="앞 글에서 이어짐, 판독 불확실"></label></div><label>1. 원문과 음독<textarea data-pj-field="original_reading"></textarea></label><label>2. 현대어 직역<textarea data-pj-field="literal_translation"></textarea></label><label>3. 현대어 의역<textarea data-pj-field="interpretive_translation"></textarea></label><label>4. 참고<textarea data-pj-field="notes"></textarea></label>`;box.querySelectorAll('[data-pj-field]').forEach(el=>{if(el.tagName==='TEXTAREA')el.value=page[el.dataset.pjField]||''})}
  function pjReset(){ $('#pyeongjae-form').reset();$('#pj-id').value='';$('#pj-form-title').textContent='새 장 등록';$('#pj-book').value='1';$('#pj-sheet').value='1';pjPages=[pjBlankPage('front',1),pjBlankPage('back',1)];pjPageIndex=0;$('#pj-message').textContent='';pjUpdateTitle();pjRenderPage() }
  function pjNormalizedPages(row){const sheet=Number(row.sheet_no??row.start_page)||1,source=Array.isArray(row.pages)?row.pages:[],find=side=>source.find(p=>p.side===side)||pjBlankPage(side,sheet);return ['front','back'].map(side=>({...pjBlankPage(side,sheet),...structuredClone(find(side)),page:sheet,side}))}
  function pjOpen(row=null){pjReset();if(row){$('#pj-id').value=row.id;$('#pj-form-title').textContent='평재문집 자료 수정';for(const [id,key] of [['pj-book','book_no'],['pj-volume','volume_no'],['pj-genre','genre'],['pj-review','review_status'],['pj-work-title','work_title'],['pj-summary','summary'],['pj-admin-note','admin_note']])$('#'+id).value=row[key]??'';$('#pj-sheet').value=row.sheet_no??row.start_page??1;$('#pj-people').value=(row.people||[]).join(', ');$('#pj-places').value=(row.places||[]).join(', ');$('#pj-tags').value=(row.tags||[]).join(', ');$('#pj-references').value=(Array.isArray(row.reference_links)?row.reference_links:[]).map(x=>`${x.label||''} | ${x.url||''}`).join('\n');pjPages=pjNormalizedPages(row);pjPageIndex=0;pjUpdateTitle();pjRenderPage()}$('#pyeongjae-form').classList.remove('hidden');window.scrollTo({top:0,behavior:'smooth'})}
  ...
  function pjPayload(status){pjSyncPage();const book=Number($('#pj-book').value),sheet=Number($('#pj-sheet').value),title=pjTitle(),volume=$('#pj-volume').value?Number($('#pj-volume').value):null;if(![1,2,3].includes(book)||!Number.isInteger(sheet)||sheet<1)throw new Error('책 번호와 장 번호를 확인해 주세요.');const duplicate=pyeongjaeEntries.find(r=>r.id!==$('#pj-id').value&&Number(r.book_no)===book&&Number(r.sheet_no??r.start_page)===sheet);if(duplicate)throw new Error(`제${book}책 제${sheet}장은 이미 등록되어 있습니다.`);if(status==='published'&&!pjPages.some(p=>p.original_reading||p.literal_translation||p.interpretive_translation||p.notes))throw new Error('공개하려면 앞면 또는 뒷면의 본문을 한 부분 이상 입력해 주세요.');return{book_no:book,sheet_no:sheet,volume_no:volume,start_page:sheet,end_page:sheet,side:'mixed',genre:$('#pj-genre').value,work_title:$('#pj-work-title').value.trim(),title,summary:$('#pj-summary').value.trim(),people:normalizeTags($('#pj-people').value),places:normalizeTags($('#pj-places').value),tags:normalizeTags($('#pj-tags').value),pages:pjPages.map(p=>({...p,page:sheet})),reference_links:pjReferences(),review_status:$('#pj-review').value,admin_note:$('#pj-admin-note').value.trim(),status,source_order:book*100000+sheet,deleted_at:null,published_at:status==='published'?(pyeongjaeEntries.find(x=>x.id===$('#pj-id').value)?.published_at||new Date().toISOString()):null}}
  function pjParsePaste(){const raw=$('#pj-paste').value;if(!raw.trim())return showToast('정리본을 먼저 붙여넣어 주세요.',true);const modern=raw.match(/■\s*제\s*([1-3])책\s*제\s*(\d+)장\s*\((앞|뒤|뒷)\)/),legacy=raw.match(/■\s*권\s*(\d+)[_\s]*(\d+)쪽\s*\((앞|뒤|뒷)\)/),header=modern||legacy;const section=(n,next)=>{const re=new RegExp(`${n}\\.\\s*[^\\n]*\\n([\\s\\S]*?)(?=\\n${next}\\.|$)`);return raw.match(re)?.[1]?.trim()||''};if(modern){$('#pj-book').value=modern[1];$('#pj-sheet').value=modern[2]}else if(legacy){$('#pj-volume').value=legacy[1];$('#pj-sheet').value=legacy[2]}const side=/뒤|뒷/.test(header?.[3]||'')?'back':'front',index=side==='back'?1:0,page={...pjPages[index],page:Number($('#pj-sheet').value)||1,side};page.original_reading=section(1,2);page.literal_translation=section(2,3);page.interpretive_translation=section(3,4);page.notes=raw.match(/4\.\s*[^\n]*\n([\s\S]*)/)?.[1]?.trim()||'';pjPages[index]=page;pjPageIndex=index;pjUpdateTitle();pjRenderPage();showToast(`${side==='back'?'뒷면':'앞면'} 영역을 나누었습니다. 저장 전에 내용을 확인해 주세요.`)}
  function pjDuplicateWarning(){if(!$('#pj-message'))return;const id=$('#pj-id').value,book=Number($('#pj-book').value),sheet=Number($('#pj-sheet').value),duplicate=pyeongjaeEntries.find(r=>r.id!==id&&Number(r.book_no)===book&&Number(r.sheet_no??r.start_page)===sheet);$('#pj-message').textContent=duplicate?`제${book}책 제${sheet}장은 이미 등록되어 있습니다. 기존 자료를 수정해 주세요.`:''}
  function renderPyeongjaeTable(){const term=$('#pj-search').value.trim().toLocaleLowerCase('ko-KR'),filter=$('#pj-filter').value,book=$('#pj-book-filter').value,genre=$('#pj-genre-filter').value,review=$('#pj-review-filter').value,sort=$('#pj-sort').value;let rows=pyeongjaeEntries.filter(r=>(filter==='all'||filter==='active'?filter!=='active'||r.status!=='trashed':r.status===filter)&&(book==='all'||Number(r.book_no)===Number(book))&&(genre==='all'||r.genre===genre)&&(review==='all'||r.review_status===review)&&(!term||[r.title,r.work_title,...(r.people||[]),...(r.places||[]),...(r.tags||[])].join(' ').toLocaleLowerCase('ko-KR').includes(term)));rows.sort((a,b)=>sort==='updated'?new Date(b.updated_at)-new Date(a.updated_at):sort==='views'?Number(b.view_count)-Number(a.view_count):pjBookSheetCompare(a,b));$('#pj-empty').classList.toggle('hidden',!!rows.length);$('#pj-table').innerHTML=rows.map(r=>`<tr data-pj-id="${r.id}"><td><strong>제${r.book_no}책<br>제${r.sheet_no??r.start_page}장</strong></td><td><strong class="pj-admin-title">${escapeText(r.genre)} · ${escapeText(r.work_title||r.title)}<small>${escapeText([...(r.people||[]),...(r.tags||[])].join(' · '))}</small></strong></td><td>${escapeText(r.review_status)}</td><td><span class="badge ${r.status==='published'?'published':r.status==='trashed'?'trashed':'draft'}">${statusLabels[r.status]||r.status}</span></td><td>${formatDate(r.updated_at)}<br><small>조회 ${Number(r.view_count||0).toLocaleString()}</small></td><td><div class="guide-actions">${r.status!=='trashed'?'<button class="button secondary small" data-pj-action="edit">수정</button><button class="button ghost small" data-pj-action="open">보기</button><button class="button ghost small" data-pj-action="toggle">공개 전환</button><button class="button danger small" data-pj-action="trash">휴지통</button>':'<button class="button secondary small" data-pj-action="restore">복원</button><button class="button danger small" data-pj-action="delete">완전 삭제</button>'}</div></td></tr>`).join('')}
  async function loadPyeongjae(){const{data,error}=await db.from('pyeongjae_entries').select('*').order('book_no').order('sheet_no');pyeongjaeEntries=error?[]:(data||[]);if($('#pj-table'))renderPyeongjaeTable()}
  $('#new-pyeongjae')?.addEventListener('click',()=>{try{pjOpen()}catch(error){console.error('평재문집 새 장 양식 열기 실패',error);showToast('새 장 등록 화면을 열지 못했습니다. 새로고침 후 다시 시도해 주세요.',true)}});$('#pj-cancel').addEventListener('click',()=>$('#pyeongjae-form').classList.add('hidden'));$('#pj-page-tabs').addEventListener('click',e=>{const i=e.target.dataset.pjPage;if(i===undefined)return;pjSyncPage();pjPageIndex=Number(i);pjRenderPage()});$('#pj-parse').addEventListener('click',pjParsePaste);['#pj-book','#pj-sheet'].forEach(sel=>$(sel).addEventListener('input',pjUpdateTitle));$('#pj-genre').addEventListener('change',()=>{pjPages.forEach(p=>p.genre=$('#pj-genre').value);pjRenderPage()});['#pj-search','#pj-book-filter','#pj-genre-filter','#pj-review-filter','#pj-filter','#pj-sort'].forEach(sel=>$(sel).addEventListener(sel==='#pj-search'?'input':'change',renderPyeongjaeTable));
  $('#pyeongjae-form').addEventListener('submit',async e=>{e.preventDefault();const button=e.submitter||e.currentTarget.querySelector('[data-status="draft"]'),status=button.dataset.status||'draft',id=$('#pj-id').value,message=$('#pj-message');let payload;try{payload=pjPayload(status)}catch(err){message.textContent=err.message;return}button.disabled=true;const result=id?await db.from('pyeongjae_entries').update(payload).eq('id',id):await db.from('pyeongjae_entries').insert({...payload,created_by:currentUser.id});button.disabled=false;if(result.error){message.textContent=/unique|duplicate/i.test(result.error.message)?`제${payload.book_no}책 제${payload.sheet_no}장은 이미 등록되어 있습니다.`:result.error.message;return}$('#pyeongjae-form').classList.add('hidden');await loadPyeongjae();showToast(status==='published'?'공개 자료를 저장했습니다.':'임시저장했습니다.')});
  $('#pj-preview').addEventListener('click',()=>{try{const row=pjPayload('draft'),out=$('#guide-preview-content');out.replaceChildren();const h=document.createElement('h1'),m=document.createElement('p'),b=document.createElement('div');h.textContent=row.work_title||row.title;m.textContent=`『평재문집』 제${row.book_no}책 제${row.sheet_no}장 앞·뒷면 · ${row.genre}`;b.style.whiteSpace='pre-wrap';b.style.lineHeight='1.7';b.textContent=row.pages.map(p=>`■ 제${row.book_no}책 제${row.sheet_no}장(${p.side==='front'?'앞':'뒤'})\n\n1. 원문과 음독\n${p.original_reading}\n\n2. 현대어 직역\n${p.literal_translation}\n\n3. 현대어 의역\n${p.interpretive_translation}\n\n4. 참고\n${p.notes}`).join('\n\n');out.append(h,m,b);$('#guide-preview-dialog').showModal()}catch(err){showToast(err.message,true)}});
