(() => {
  ...
  const pjTitle=()=>$('#pj-free-title').checked?($('#pj-custom-title').value.trim()||pjAutoTitle()):pjAutoTitle();
  const pjFaceOrder=v=>v==='back'?2:1;
  const pjPriorityRank=row=>String(row?.title||'').trimStart().startsWith('○')?0:String(row?.title||'').trimStart().startsWith('#')?1:2;
  const pjIsPriority=row=>pjPriorityRank(row)<2;
  ...
  const pjBookSheetCompare=(a,b)=>pjPriorityCompare(a,b)||(Number(a.book_no)-Number(b.book_no))||(Number(a.sheet_no??a.start_page)-Number(b.sheet_no??b.start_page))||(pjFaceOrder(a.side)-pjFaceOrder(b.side));
  ...
  function pjUpdateTitle(){const sheet=Number($('#pj-sheet').value)||1,side=$('#pj-side').value||'front';$('#pj-custom-title-wrap').classList.toggle('hidden',!$('#pj-free-title').checked);$('#pj-title').value=pjTitle();if(pjPages[0])Object.assign(pjPages[0],{page:sheet,side});pjDuplicateWarning()}
  ...
  function pjReset(){ $('#pyeongjae-form').reset();$('#pj-id').value='';$('#pj-form-title').textContent='새 자료 등록';$('#pj-book').value='1';$('#pj-sheet').value='1';$('#pj-side').value='front';pjPages=[pjBlankPage('front',1)];pjPageIndex=0;$('#pj-message').textContent='';pjUpdateTitle();pjRenderPage() }
  ...
  function pjOpen(row=null){pjReset();if(row){$('#pj-id').value=row.id;$('#pj-form-title').textContent='평재문집 자료 수정';$('#pj-book').value=row.book_no??1;$('#pj-sheet').value=row.sheet_no??row.start_page??1;$('#pj-side').value=row.side==='back'?'back':'front';$('#pj-volume').value=row.volume_no??'';$('#pj-genre').value=row.genre||'기타';$('#pj-tags').value=(row.tags||[]).join(', ');$('#pj-admin-note').value=row.admin_note||'';const auto=`평재문집 제${row.book_no}책 제${row.sheet_no??row.start_page}장 ${row.side==='back'?'뒷면':'앞면'}`,custom=Boolean(row.title&&row.title!==auto);$('#pj-free-title').checked=custom;$('#pj-custom-title').value=custom?row.title:'';pjPages=[pjPageForRow(row)];pjUpdateTitle();pjRenderPage()}$('#pyeongjae-form').classList.remove('hidden');window.scrollTo({top:0,behavior:'smooth'})}
  ...
  function pjPayload(status){pjSyncPage();const book=Number($('#pj-book').value),sheet=Number($('#pj-sheet').value),side=$('#pj-side').value,volume=$('#pj-volume').value?Number($('#pj-volume').value):null,current=pyeongjaeEntries.find(x=>x.id===$('#pj-id').value);if(![1,2,3].includes(book)||!Number.isInteger(sheet)||sheet<1||!['front','back'].includes(side))throw new Error('책 번호·장 번호·면을 확인해 주세요.');const duplicate=pyeongjaeEntries.find(r=>r.id!==$('#pj-id').value&&Number(r.book_no)===book&&Number(r.sheet_no??r.start_page)===sheet&&r.side===side);if(duplicate)throw new Error(`제${book}책 제${sheet}장 ${side==='back'?'뒷면':'앞면'}은 이미 등록되어 있습니다.`);if($('#pj-free-title').checked&&!$('#pj-custom-title').value.trim())throw new Error('자유 제목을 입력해 주세요.');if(status==='published'&&!pjPages.some(p=>p.original_reading||p.literal_translation||p.interpretive_translation||p.notes))throw new Error('공개하려면 본문을 한 부분 이상 입력해 주세요.');const page={...pjPages[0],page:sheet,side,genre:$('#pj-genre').value};return{book_no:book,sheet_no:sheet,volume_no:volume,start_page:sheet,end_page:sheet,side,genre:$('#pj-genre').value,work_title:current?.work_title||'',title:pjTitle(),summary:current?.summary||'',people:current?.people||[],places:current?.places||[],tags:normalizeTags($('#pj-tags').value),pages:[page],reference_links:pjReferences(),review_status:current?.review_status||'초벌 정리',admin_note:$('#pj-admin-note').value.trim(),status,source_order:book*100000+sheet*10+pjFaceOrder(side),deleted_at:null,published_at:status==='published'?(current?.published_at||new Date().toISOString()):null}}
  ...
  function pjDuplicateWarning(){if(!$('#pj-message'))return;const id=$('#pj-id').value,book=Number($('#pj-book').value),sheet=Number($('#pj-sheet').value),side=$('#pj-side').value,duplicate=pyeongjaeEntries.find(r=>r.id!==id&&Number(r.book_no)===book&&Number(r.sheet_no??r.start_page)===sheet&&r.side===side);$('#pj-message').textContent=duplicate?`제${book}책 제${sheet}장 ${side==='back'?'뒷면':'앞면'}은 이미 등록되어 있습니다.`:''}
  function renderPyeongjaeTable(){const term=$('#pj-search').value.trim().toLocaleLowerCase('ko-KR'),filter=$('#pj-filter').value,book=$('#pj-book-filter').value,genre=$('#pj-genre-filter').value,review=$('#pj-review-filter').value,sort=$('#pj-sort').value;let rows=pyeongjaeEntries.filter(r=>(filter==='all'||filter==='active'?filter!=='active'||r.status!=='trashed':r.status===filter)&&(book==='all'||Number(r.book_no)===Number(book))&&(genre==='all'||r.genre===genre)&&(review==='all'||r.review_status===review)&&(!term||[r.title,r.genre,...(r.tags||[])].join(' ').toLocaleLowerCase('ko-KR').includes(term)));rows.sort((a,b)=>pjPriorityCompare(a,b)||(sort==='updated'?new Date(b.updated_at)-new Date(a.updated_at):sort==='views'?Number(b.view_count)-Number(a.view_count):pjBookSheetCompare(a,b)));$('#pj-empty').classList.toggle('hidden',!!rows.length);$('#pj-table').innerHTML=rows.map(r=>`<tr data-pj-id="${r.id}"><td><strong>제${r.book_no}책<br>제${r.sheet_no??r.start_page}장 ${r.side==='back'?'뒷면':'앞면'}</strong></td><td><strong class="pj-admin-title">${escapeText(r.title)}<small>${escapeText([r.volume_no?`권${r.volume_no}`:'권차 미확인',r.genre,...(r.tags||[])].join(' · '))}</small></strong></td><td>${escapeText(r.review_status)}</td><td><span class="badge ${r.status==='published'?'published':r.status==='trashed'?'trashed':'draft'}">${statusLabels[r.status]||r.status}</span></td><td>${formatDate(r.updated_at)}<br><small>조회 ${Number(r.view_count||0).toLocaleString()}</small></td><td><div class="guide-actions">${r.status!=='trashed'?'<button class="button secondary small" data-pj-action="edit">수정</button><button class="button ghost small" data-pj-action="open">보기</button><button class="button ghost small" data-pj-action="toggle">공개 전환</button><button class="button danger small" data-pj-action="trash">휴지통</button>':'<button class="button secondary small" data-pj-action="restore">복원</button><button class="button danger small" data-pj-action="delete">완전 삭제</button>'}</div></td></tr>`).join('')}
  ...
  $('#new-pyeongjae')?.addEventListener('click',()=>{try{pjOpen()}catch(error){console.error('평재문집 새 자료 양식 열기 실패',error);showToast('새 자료 등록 화면을 열지 못했습니다.',true)}});$('#pj-cancel').addEventListener('click',()=>$('#pyeongjae-form').classList.add('hidden'));$('#pj-parse').addEventListener('click',pjParsePaste);['#pj-book','#pj-sheet','#pj-side'].forEach(sel=>$(sel).addEventListener('input',()=>{pjUpdateTitle();pjRenderPage()}));$('#pj-free-title').addEventListener('change',pjUpdateTitle);$('#pj-custom-title').addEventListener('input',pjUpdateTitle);['#pj-search','#pj-book-filter','#pj-genre-filter','#pj-review-filter','#pj-filter','#pj-sort'].forEach(sel=>$(sel).addEventListener(sel==='#pj-search'?'input':'change',renderPyeongjaeTable));
  ...
  $('#pj-preview').addEventListener('click',()=>{try{const row=pjPayload('draft'),out=$('#guide-preview-content');out.replaceChildren();const h=document.createElement('h1'),m=document.createElement('p'),b=document.createElement('div');h.textContent=row.title;m.textContent=`${row.volume_no?`권${row.volume_no} · `:''}${row.genre}`;b.style.whiteSpace='pre-wrap';b.style.lineHeight='1.7';const p=row.pages[0];b.textContent=`원문과 음독\n${p.original_reading}\n\n현대어 직역\n${p.literal_translation}${p.interpretive_translation?`\n\n현대어 의역\n${p.interpretive_translation}`:''}\n\n참고\n${p.notes}`;out.append(h,m,b);$('#guide-preview-dialog').showModal()}catch(err){showToast(err.message,true)}});
