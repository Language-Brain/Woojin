(() => {
  ...
  function sanitizeGuideHtml(value){const cleaned=DOMPurify.sanitize(String(value||''),{ALLOWED_TAGS:guideAllowedTags,ALLOWED_ATTR:['href','target','rel','style']});const box=document.createElement('div');box.innerHTML=cleaned;normalizeGuideBody(box);return box.innerHTML}
  function setGuideBody(value){const raw=String(value||''),isHtml=/<\/?(?:p|br|div|strong|b|u|span|a|ol|ul|li)\b/i.test(raw);guideEditor.innerHTML=sanitizeGuideHtml(isHtml?raw:guidePlainToHtml(raw));$('#guide-body').value=guideEditor.innerHTML}
  function linkifyGuideUrls(root){const nodes=[],walker=document.createTreeWalker(root,NodeFilter.SHOW_TEXT);while(walker.nextNode())if(!walker.currentNode.parentElement?.closest('a')&&/https?:\/\//i.test(walker.currentNode.nodeValue||''))nodes.push(walker.currentNode);nodes.forEach(node=>{const text=node.nodeValue||'',re=/https?:\/\/[^\s<>"']+/gi,frag=document.createDocumentFragment();let match,last=0,changed=false;while((match=re.exec(text))){let shown=match[0],tail='';while(shown&&'.,!?;:)]}。、'.includes(shown.at(-1))){tail=shown.at(-1)+tail;shown=shown.slice(0,-1)}const safe=guideSafeUrl(shown);if(!safe)continue;frag.append(document.createTextNode(text.slice(last,match.index)));const a=document.createElement('a');a.href=safe.href;a.target='_blank';a.rel='noopener noreferrer';a.textContent=shown;frag.append(a,document.createTextNode(tail));last=match.index+match[0].length;changed=true}if(changed){frag.append(document.createTextNode(text.slice(last)));node.replaceWith(frag)}})}
  function guideBodyHtml(){linkifyGuideUrls(guideEditor);const html=sanitizeGuideHtml(guideEditor.innerHTML);$('#guide-body').value=html;return html}
  function restoreGuideSelection(){if(!guideSelection)return false;const selection=getSelection();selection.removeAllRanges();selection.addRange(guideSelection);return true}
  function rememberGuideSelection(){const selection=getSelection();if(selection.rangeCount&&guideEditor.contains(selection.anchorNode))guideSelection=selection.getRangeAt(0).cloneRange()}
  function updateGuideToolbar(){['bold','underline'].forEach(command=>document.querySelector(`[data-guide-command="${command}"]`)?.classList.toggle('is-active',document.queryCommandState(command)))}
  function unlinkGuideSelection(){if(!restoreGuideSelection())guideEditor.focus();document.execCommand('unlink');guideEditor.focus()}
  function openGuideLinkDialog(){rememberGuideSelection();const selected=guideSelection?.commonAncestorContainer?.parentElement?.closest?.('a'),dialog=$('#guide-link-dialog');$('#guide-link-url').value=selected?.href||'';$('#guide-link-new-tab').checked=!selected||selected.target==='_blank';$('#guide-link-message').textContent=guideSelection?.collapsed?'링크를 적용할 글자를 먼저 선택해 주세요.':'';dialog.showModal();$('#guide-link-url').focus()}
  function applyGuideLink(){if(!guideSelection||guideSelection.collapsed)return $('#guide-link-message').textContent='링크를 적용할 글자를 먼저 선택해 주세요.';const safe=guideSafeUrl($('#guide-link-url').value);if(!safe)return $('#guide-link-message').textContent='http:// 또는 https://로 시작하는 올바른 주소를 입력해 주세요.';restoreGuideSelection();document.execCommand('createLink',false,safe.href);const anchor=getSelection().anchorNode?.parentElement?.closest?.('a');if(anchor){if($('#guide-link-new-tab').checked){anchor.target='_blank';anchor.rel='noopener noreferrer'}else{anchor.removeAttribute('target');anchor.removeAttribute('rel')}}$('#guide-link-dialog').close();guideEditor.focus()}
  ...
  function previewGuide(row){const out=$('#guide-preview-content');out.replaceChildren();const h=document.createElement('h1'),meta=document.createElement('p'),desc=document.createElement('p'),body=document.createElement('div');h.textContent=row.title||'제목 없음';meta.textContent=(row.tags||[]).map(t=>'#'+t).join(' ');desc.textContent=row.description||'';body.className='guide-preview-body';body.style.lineHeight='1.8';body.innerHTML=sanitizeGuideHtml(row.body||'');out.append(h,meta,desc,body);$('#guide-preview-dialog').showModal()}
  ...
  $('#new-guide').addEventListener('click',()=>openGuideForm());$('#guide-cancel').addEventListener('click',()=>$('#guide-form').classList.add('hidden'));$('#add-guide-link').addEventListener('click',()=>addGuideLinkRow());$('#guide-links-editor').addEventListener('click',e=>{if(e.target.matches('[data-remove-guide-link]'))e.target.closest('.book-link-row').remove()});$('#close-guide-preview').addEventListener('click',()=>$('#guide-preview-dialog').close());$('#guide-preview').addEventListener('click',()=>{try{previewGuide({title:$('#guide-title').value,description:$('#guide-description').value,body:guideBodyHtml(),tags:normalizeTags($('#guide-tags').value)})}catch(e){showToast(e.message,true)}});['#guide-admin-search','#guide-admin-filter','#guide-admin-sort'].forEach(selector=>$(selector).addEventListener(selector.includes('search')?'input':'change',renderGuideTable));
  $('#guide-body-toolbar').addEventListener('mousedown',event=>{if(event.target.closest('button'))event.preventDefault()});$('#guide-body-toolbar').addEventListener('click',event=>{const button=event.target.closest('[data-guide-command]');if(!button)return;const command=button.dataset.guideCommand;guideEditor.focus();if(command==='unlink')unlinkGuideSelection();else document.execCommand(command,false,null);rememberGuideSelection();updateGuideToolbar()});$('#guide-text-color').addEventListener('input',event=>{restoreGuideSelection();document.execCommand('foreColor',false,event.target.value);guideEditor.focus()});$('#guide-link-open').addEventListener('click',openGuideLinkDialog);$('#guide-link-apply').addEventListener('click',applyGuideLink);$('#guide-link-remove').addEventListener('click',()=>{unlinkGuideSelection();$('#guide-link-dialog').close()});guideEditor.addEventListener('mouseup',()=>{rememberGuideSelection();updateGuideToolbar()});guideEditor.addEventListener('keyup',()=>{rememberGuideSelection();updateGuideToolbar()});guideEditor.addEventListener('paste',event=>{event.preventDefault();const html=event.clipboardData.getData('text/html'),plain=event.clipboardData.getData('text/plain');document.execCommand('insertHTML',false,html?sanitizeGuideHtml(html):guidePlainToHtml(plain));requestAnimationFrame(()=>{linkifyGuideUrls(guideEditor);rememberGuideSelection()})});
  $('#guide-form').addEventListener('submit',async e=>{e.preventDefault();const message=$('#guide-message'),id=$('#guide-id').value;let links;try{links=guideLinks()}catch(error){return message.textContent=error.message}const youtube=$('#guide-youtube').value.trim();if(youtube&&!cleanBookUrl(youtube))return message.textContent='올바른 YouTube 주소를 입력해 주세요.';const body=guideBodyHtml(),bodyText=guideEditor.textContent.trim();const payload={title:$('#guide-title').value.trim(),description:$('#guide-description').value.trim(),body,course_name:$('#guide-course').value.trim(),institution_name:$('#guide-institution').value.trim(),tags:normalizeTags($('#guide-tags').value),external_links:links,youtube_url:youtube||null,visibility:$('#guide-visibility').value,status:'active',deleted_at:null};if(!payload.title||!bodyText)return message.textContent='제목과 본문을 입력해 주세요.';e.submitter.disabled=true;const result=id?await db.from('guides').update(payload).eq('id',id):await db.from('guides').insert({...payload,created_by:currentUser.id});e.submitter.disabled=false;if(result.error)return message.textContent=result.error.message;$('#guide-form').classList.add('hidden');await loadGuides();showToast('자료를 저장했습니다.')});
  ...
  const PJ_GENRES=['시','서간','편지','잡저','기문','축문','제문','애사','묘지명','행장','복합·경계','기타'];
  const pjGuideEditor=$('#pj-guide-editor'),pjGuideAllowed=['p','br','div','strong','b','u','em','i','span','a','ol','ul','li','table','thead','tbody','tr','th','td','img','h2','h3','blockquote'];
  const pjEntryKind=()=>document.querySelector('input[name="pj-entry-kind"]:checked')?.value||'source';
  const pjIsGuideRow=row=>row?.entry_kind==='guide';
  function pjSanitizeGuide(value){const cleaned=DOMPurify.sanitize(String(value||''),{ALLOWED_TAGS:pjGuideAllowed,ALLOWED_ATTR:['href','target','rel','style','src','alt','title','colspan','rowspan']});const box=document.createElement('div');box.innerHTML=cleaned;box.querySelectorAll('[style]').forEach(node=>{const color=node.style.color;node.removeAttribute('style');if(color&&CSS.supports('color',color))node.style.color=color});box.querySelectorAll('a').forEach(link=>{const safe=guideSafeUrl(link.getAttribute('href'));if(!safe)return link.replaceWith(...link.childNodes);link.href=safe.href;if(safe.origin!==location.origin||link.target==='_blank'){link.target='_blank';link.rel='noopener noreferrer'}else{link.removeAttribute('target');link.removeAttribute('rel')}});box.querySelectorAll('img').forEach(image=>{const safe=guideSafeUrl(image.getAttribute('src'));if(!safe)return image.remove();image.src=safe.href;image.alt=String(image.alt||'').slice(0,300);image.removeAttribute('title')});return box.innerHTML}
  function pjGuideBody(){const html=pjSanitizeGuide(pjGuideEditor.innerHTML);pjGuideEditor.innerHTML=html;return html}
  const pjBlankPage=(side='front',sheet=1)=>({page:sheet,side,genre:$('#pj-genre')?.value||'기타',work_title:'',marker:'',original_reading:'',literal_translation:'',interpretive_translation:'',notes:''});
  const pjAutoTitle=()=>`평재문집 제${Number($('#pj-book').value)||1}책 제${Number($('#pj-sheet').value)||1}장 ${$('#pj-side').value==='back'?'뒷면':'앞면'}`;
  const pjTitle=()=>pjEntryKind()==='guide'?$('#pj-custom-title').value.trim():$('#pj-free-title').checked?($('#pj-custom-title').value.trim()||pjAutoTitle()):pjAutoTitle();
  const pjFaceOrder=v=>v==='back'?2:1;
  const pjPriorityRank=row=>{const title=String(row?.title||'').trimStart();if(pjIsGuideRow(row)||title.startsWith('○')||title.startsWith('#'))return title.startsWith('○')?0:title.startsWith('#')?1:2;return 3};
  const pjIsPriority=row=>pjPriorityRank(row)<3;
  const pjCreatedTime=row=>Date.parse(row?.created_at||row?.published_at||row?.updated_at||'')||0;
  const pjPriorityCompare=(a,b)=>pjPriorityRank(a)-pjPriorityRank(b)||(pjIsPriority(a)&&pjIsPriority(b)?pjCreatedTime(a)-pjCreatedTime(b):0);
  const pjBookSheetCompare=(a,b)=>pjPriorityCompare(a,b)||(Number(a.book_no||0)-Number(b.book_no||0))||(Number(a.sheet_no??a.start_page??0)-Number(b.sheet_no??b.start_page??0))||(pjFaceOrder(a.side)-pjFaceOrder(b.side));
  function pjSyncPage(){const box=$('#pj-page-editor');if(!box||!pjPages[0])return;box.querySelectorAll('[data-pj-field]').forEach(el=>pjPages[0][el.dataset.pjField]=el.value)}
  function pjUpdateMode(){const guide=pjEntryKind()==='guide';$('#pj-source-fields').classList.toggle('hidden',guide);$('#pj-source-content').classList.toggle('hidden',guide);$('#pj-guide-content').classList.toggle('hidden',!guide);$('#pj-auto-title-wrap').classList.toggle('hidden',guide);$('.pj-free-title').classList.toggle('hidden',guide);$('#pj-custom-title-wrap').classList.toggle('hidden',!guide&&!$('#pj-free-title').checked);['#pj-book','#pj-sheet','#pj-side','#pj-genre'].forEach(selector=>$(selector).required=!guide);if(guide)$('#pj-free-title').checked=true;pjUpdateTitle()}
  function pjUpdateTitle(){const guide=pjEntryKind()==='guide',sheet=Number($('#pj-sheet').value)||1,side=$('#pj-side').value||'front';$('#pj-custom-title-wrap').classList.toggle('hidden',!guide&&!$('#pj-free-title').checked);if(!guide)$('#pj-title').value=pjTitle();if(pjPages[0]&&!guide)Object.assign(pjPages[0],{page:sheet,side});pjDuplicateWarning()}
  function pjRenderPage(){const box=$('#pj-page-editor'),sheet=Number($('#pj-sheet').value)||1,side=$('#pj-side').value||'front',page=pjPages[0]||pjBlankPage(side,sheet);$('#pj-page-tabs').innerHTML=`<span class="pj-face-label">${side==='back'?'뒷면':'앞면'}</span>`;box.innerHTML=`<div class="pj-page-meta"><strong>제${sheet}장 ${side==='back'?'뒷면':'앞면'}</strong></div><details class="pj-text-section" open><summary>원문과 음독</summary><textarea aria-label="원문과 음독" data-pj-field="original_reading"></textarea></details><details class="pj-text-section" open><summary>현대어 직역</summary><textarea aria-label="현대어 직역" data-pj-field="literal_translation"></textarea></details><details class="pj-text-section"><summary>현대어 의역 <small>필요하지 않으면 비워 두세요.</small></summary><textarea aria-label="현대어 의역" data-pj-field="interpretive_translation"></textarea></details><details class="pj-text-section"><summary>참고</summary><textarea aria-label="참고" data-pj-field="notes"></textarea></details>`;box.querySelectorAll('[data-pj-field]').forEach(el=>el.value=page[el.dataset.pjField]||'')}
  function pjReset(){ $('#pyeongjae-form').reset();$('#pj-id').value='';$('#pj-form-title').textContent='새 자료 등록';document.querySelector('input[name="pj-entry-kind"][value="source"]').checked=true;$('#pj-book').value='1';$('#pj-sheet').value='1';$('#pj-side').value='front';pjGuideEditor.innerHTML='';pjPages=[pjBlankPage('front',1)];pjPageIndex=0;$('#pj-message').textContent='';pjUpdateMode();pjRenderPage() }
  function pjPageForRow(row){const sheet=Number(row.sheet_no??row.start_page)||1,side=row.side==='back'?'back':'front',source=Array.isArray(row.pages)?row.pages:[],page=source.find(p=>p.side===side)||source.find(p=>[p.original_reading,p.literal_translation,p.interpretive_translation,p.notes].some(Boolean))||{};return{...pjBlankPage(side,sheet),...structuredClone(page),page:sheet,side}}
  function pjOpen(row=null){pjReset();if(row){const guide=pjIsGuideRow(row);document.querySelector(`input[name="pj-entry-kind"][value="${guide?'guide':'source'}"]`).checked=true;$('#pj-id').value=row.id;$('#pj-form-title').textContent=guide?'안내 글 수정':'평재문집 원문 수정';$('#pj-book').value=row.book_no??1;$('#pj-sheet').value=row.sheet_no??row.start_page??1;$('#pj-side').value=row.side==='back'?'back':'front';$('#pj-volume').value=row.volume_no??'';$('#pj-genre').value=row.genre||'기타';$('#pj-tags').value=(row.tags||[]).join(', ');$('#pj-admin-note').value=row.admin_note||'';const auto=guide?'':`평재문집 제${row.book_no}책 제${row.sheet_no??row.start_page}장 ${row.side==='back'?'뒷면':'앞면'}`,custom=guide||Boolean(row.title&&row.title!==auto);$('#pj-free-title').checked=custom;$('#pj-custom-title').value=custom?row.title:'';pjGuideEditor.innerHTML=pjSanitizeGuide(row.guide_body||'');pjPages=[pjPageForRow(row)];pjUpdateMode();pjRenderPage()}$('#pyeongjae-form').classList.remove('hidden');(pjEntryKind()==='guide'?$('#pj-custom-title'):$('#pj-book')).focus();window.scrollTo({top:0,behavior:'smooth'})}
  ...
  function pjPayload(status){const guide=pjEntryKind()==='guide',current=pyeongjaeEntries.find(x=>x.id===$('#pj-id').value);if(guide){const title=$('#pj-custom-title').value.trim(),body=pjGuideBody(),bodyText=pjGuideEditor.textContent.trim();if(!title)throw new Error('안내 글 제목을 입력해 주세요.');if(status==='published'&&!bodyText&&!pjGuideEditor.querySelector('img'))throw new Error('공개하려면 안내 글 본문을 입력해 주세요.');return{entry_kind:'guide',book_no:null,sheet_no:null,volume_no:null,start_page:null,end_page:null,side:null,genre:null,work_title:'',title,summary:'',people:[],places:[],tags:normalizeTags($('#pj-tags').value),pages:[],guide_body:body,reference_links:pjReferences(),review_status:current?.review_status||'검토 완료',admin_note:$('#pj-admin-note').value.trim(),status,source_order:0,deleted_at:null,published_at:status==='published'?(current?.published_at||new Date().toISOString()):null}}pjSyncPage();const book=Number($('#pj-book').value),sheet=Number($('#pj-sheet').value),side=$('#pj-side').value,volume=$('#pj-volume').value?Number($('#pj-volume').value):null;if(![1,2,3].includes(book)||!Number.isInteger(sheet)||sheet<1||!['front','back'].includes(side))throw new Error('책 번호·장 번호·면을 확인해 주세요.');const duplicate=pyeongjaeEntries.find(r=>!pjIsGuideRow(r)&&r.id!==$('#pj-id').value&&Number(r.book_no)===book&&Number(r.sheet_no??r.start_page)===sheet&&r.side===side);if(duplicate)throw new Error(`제${book}책 제${sheet}장 ${side==='back'?'뒷면':'앞면'}은 이미 등록되어 있습니다.`);if($('#pj-free-title').checked&&!$('#pj-custom-title').value.trim())throw new Error('자유 제목을 입력해 주세요.');if(status==='published'&&!pjPages.some(p=>p.original_reading||p.literal_translation||p.interpretive_translation||p.notes))throw new Error('공개하려면 본문을 한 부분 이상 입력해 주세요.');const page={...pjPages[0],page:sheet,side,genre:$('#pj-genre').value};return{entry_kind:'source',book_no:book,sheet_no:sheet,volume_no:volume,start_page:sheet,end_page:sheet,side,genre:$('#pj-genre').value,work_title:current?.work_title||'',title:pjTitle(),summary:current?.summary||'',people:current?.people||[],places:current?.places||[],tags:normalizeTags($('#pj-tags').value),pages:[page],guide_body:'',reference_links:pjReferences(),review_status:current?.review_status||'초벌 정리',admin_note:$('#pj-admin-note').value.trim(),status,source_order:book*100000+sheet*10+pjFaceOrder(side),deleted_at:null,published_at:status==='published'?(current?.published_at||new Date().toISOString()):null}}
  function pjParsePaste(){const raw=$('#pj-paste').value;if(!raw.trim())return showToast('정리본을 먼저 붙여넣어 주세요.',true);const modern=raw.match(/■\s*제\s*([1-3])책\s*제\s*(\d+)장\s*\((앞|뒤|뒷)\)/),legacy=raw.match(/■\s*권\s*(\d+)[_\s]*(\d+)쪽\s*\((앞|뒤|뒷)\)/),header=modern||legacy;const section=(n,next)=>{const re=new RegExp(`${n}\\.\\s*[^\\n]*\\n([\\s\\S]*?)(?=\\n${next}\\.|$)`);return raw.match(re)?.[1]?.trim()||''};if(modern){$('#pj-book').value=modern[1];$('#pj-sheet').value=modern[2];$('#pj-side').value=/뒤|뒷/.test(modern[3])?'back':'front'}else if(legacy){$('#pj-volume').value=legacy[1];$('#pj-sheet').value=legacy[2];$('#pj-side').value=/뒤|뒷/.test(legacy[3])?'back':'front'}const page=pjBlankPage($('#pj-side').value,Number($('#pj-sheet').value)||1);page.original_reading=section(1,2);page.literal_translation=section(2,3);page.interpretive_translation=section(3,4);page.notes=raw.match(/4\.\s*[^\n]*\n([\s\S]*)/)?.[1]?.trim()||'';pjPages=[page];pjUpdateTitle();pjRenderPage();showToast('현재 면의 영역을 나누었습니다. 저장 전에 내용을 확인해 주세요.')}
  function pjDuplicateWarning(){if(!$('#pj-message'))return;if(pjEntryKind()==='guide'){if($('#pj-message').textContent.includes('은 이미 등록'))$('#pj-message').textContent='';return}const id=$('#pj-id').value,book=Number($('#pj-book').value),sheet=Number($('#pj-sheet').value),side=$('#pj-side').value,duplicate=pyeongjaeEntries.find(r=>!pjIsGuideRow(r)&&r.id!==id&&Number(r.book_no)===book&&Number(r.sheet_no??r.start_page)===sheet&&r.side===side);$('#pj-message').textContent=duplicate?`제${book}책 제${sheet}장 ${side==='back'?'뒷면':'앞면'}은 이미 등록되어 있습니다.`:''}
  function renderPyeongjaeTable(){const term=$('#pj-search').value.trim().toLocaleLowerCase('ko-KR'),filter=$('#pj-filter').value,book=$('#pj-book-filter').value,genre=$('#pj-genre-filter').value,review=$('#pj-review-filter').value,sort=$('#pj-sort').value;let rows=pyeongjaeEntries.filter(r=>(filter==='all'||filter==='active'?filter!=='active'||r.status!=='trashed':r.status===filter)&&(book==='all'||(!pjIsGuideRow(r)&&Number(r.book_no)===Number(book)))&&(genre==='all'||r.genre===genre)&&(review==='all'||r.review_status===review)&&(!term||[r.title,r.genre,String(r.guide_body||'').replace(/<[^>]+>/g,' '),...(r.tags||[])].join(' ').toLocaleLowerCase('ko-KR').includes(term)));rows.sort((a,b)=>sort==='updated'?new Date(b.updated_at)-new Date(a.updated_at):sort==='views'?Number(b.view_count)-Number(a.view_count):pjBookSheetCompare(a,b));$('#pj-empty').classList.toggle('hidden',!!rows.length);$('#pj-table').innerHTML=rows.map(r=>{const guide=pjIsGuideRow(r),place=guide?'<strong class="pj-guide-row-label">안내 글</strong>':`<strong>제${r.book_no}책<br>제${r.sheet_no??r.start_page}장 ${r.side==='back'?'뒷면':'앞면'}</strong>`,meta=guide?['안내 글',...(r.tags||[])]:[r.volume_no?`권${r.volume_no}`:'권차 미확인',r.genre,...(r.tags||[])];return `<tr data-pj-id="${r.id}"><td>${place}</td><td><strong class="pj-admin-title">${escapeText(r.title)}<small>${escapeText(meta.filter(Boolean).join(' · '))}</small></strong></td><td>${escapeText(guide?'해당 없음':r.review_status)}</td><td><span class="badge ${r.status==='published'?'published':r.status==='trashed'?'trashed':'draft'}">${statusLabels[r.status]||r.status}</span></td><td>${formatDate(r.updated_at)}<br><small>조회 ${Number(r.view_count||0).toLocaleString()}</small></td><td><div class="guide-actions">${r.status!=='trashed'?'<button class="button secondary small" data-pj-action="edit">수정</button><button class="button ghost small" data-pj-action="open">보기</button><button class="button ghost small" data-pj-action="toggle">공개 전환</button><button class="button danger small" data-pj-action="trash">휴지통</button>':'<button class="button secondary small" data-pj-action="restore">복원</button><button class="button danger small" data-pj-action="delete">완전 삭제</button>'}</div></td></tr>`}).join('')}
  async function loadPyeongjae(){const{data,error}=await db.from('pyeongjae_entries').select('*').order('book_no').order('sheet_no');pyeongjaeEntries=error?[]:(data||[]).sort(pjBookSheetCompare);if($('#pj-table'))renderPyeongjaeTable()}
  $('#new-pyeongjae')?.addEventListener('click',()=>{try{pjOpen()}catch(error){console.error('평재문집 새 자료 양식 열기 실패',error);showToast('새 자료 등록 화면을 열지 못했습니다.',true)}});$('#pj-cancel').addEventListener('click',()=>$('#pyeongjae-form').classList.add('hidden'));$('#pj-parse').addEventListener('click',pjParsePaste);document.querySelectorAll('input[name="pj-entry-kind"]').forEach(input=>input.addEventListener('change',pjUpdateMode));['#pj-book','#pj-sheet','#pj-side'].forEach(sel=>$(sel).addEventListener('input',()=>{pjUpdateTitle();pjRenderPage()}));$('#pj-free-title').addEventListener('change',pjUpdateTitle);$('#pj-custom-title').addEventListener('input',pjUpdateTitle);['#pj-search','#pj-book-filter','#pj-genre-filter','#pj-review-filter','#pj-filter','#pj-sort'].forEach(sel=>$(sel).addEventListener(sel==='#pj-search'?'input':'change',renderPyeongjaeTable));
  $('#pj-guide-toolbar').addEventListener('mousedown',event=>{if(event.target.closest('button'))event.preventDefault()});$('#pj-guide-toolbar').addEventListener('click',event=>{const button=event.target.closest('[data-pj-guide-command]');if(!button)return;pjGuideEditor.focus();document.execCommand(button.dataset.pjGuideCommand,false,null)});$('#pj-guide-color').addEventListener('input',event=>{pjGuideEditor.focus();document.execCommand('styleWithCSS',false,true);document.execCommand('foreColor',false,event.target.value)});$('#pj-guide-link').addEventListener('click',()=>{const selection=getSelection();if(!selection.rangeCount||selection.isCollapsed||!pjGuideEditor.contains(selection.anchorNode))return showToast('링크를 적용할 글자를 먼저 선택해 주세요.',true);const value=prompt('연결할 웹 주소를 입력해 주세요.','https://');const safe=guideSafeUrl(value);if(!safe)return value!==null&&showToast('올바른 http 또는 https 주소를 입력해 주세요.',true);document.execCommand('createLink',false,safe.href);const link=selection.anchorNode?.parentElement?.closest?.('a');if(link){link.target='_blank';link.rel='noopener noreferrer'}});$('#pj-guide-table').addEventListener('click',()=>{pjGuideEditor.focus();document.execCommand('insertHTML',false,'<table><tbody><tr><th>항목</th><th>내용</th></tr><tr><td><br></td><td><br></td></tr></tbody></table><p><br></p>')});$('#pj-guide-image').addEventListener('click',()=>$('#pj-guide-image-input').click());$('#pj-guide-image-input').addEventListener('change',async event=>{const file=event.target.files[0];if(!file)return;try{const alt=prompt('이미지를 설명하는 대체 텍스트를 입력해 주세요.','')??'',uploaded=await uploadImage(file,alt);pjGuideEditor.focus();document.execCommand('insertHTML',false,`<img src="${escapeText(uploaded.publicUrl)}" alt="${escapeText(alt)}"><p><br></p>`);showToast('안내 글에 이미지를 넣었습니다.')}catch(error){showToast(error.message,true)}finally{event.target.value=''}});
  $('#pyeongjae-form').addEventListener('submit',async e=>{e.preventDefault();const button=e.submitter||e.currentTarget.querySelector('[data-status="draft"]'),status=button.dataset.status||'draft',id=$('#pj-id').value,message=$('#pj-message');let payload;try{payload=pjPayload(status)}catch(err){message.textContent=err.message;return}button.disabled=true;const result=id?await db.from('pyeongjae_entries').update(payload).eq('id',id):await db.from('pyeongjae_entries').insert({...payload,created_by:currentUser.id});button.disabled=false;if(result.error){message.textContent=/unique|duplicate/i.test(result.error.message)?`${payload.title}은 이미 등록되어 있습니다.`:result.error.message;return}$('#pyeongjae-form').classList.add('hidden');await loadPyeongjae();showToast(status==='published'?'공개 자료를 저장했습니다.':'임시저장했습니다.')});
  $('#pj-preview').addEventListener('click',()=>{try{const row=pjPayload('draft'),out=$('#guide-preview-content');out.replaceChildren();const h=document.createElement('h1'),m=document.createElement('p'),b=document.createElement('div');h.textContent=row.title;if(row.entry_kind==='guide'){m.textContent='『평재문집』 안내 글';b.className='guide-preview-body';b.innerHTML=pjSanitizeGuide(row.guide_body)}else{m.textContent=`${row.volume_no?`권${row.volume_no} · `:''}${row.genre}`;b.style.whiteSpace='pre-wrap';b.style.lineHeight='1.7';const p=row.pages[0];b.textContent=`원문과 음독\n${p.original_reading}\n\n현대어 직역\n${p.literal_translation}${p.interpretive_translation?`\n\n현대어 의역\n${p.interpretive_translation}`:''}\n\n참고\n${p.notes}`}out.append(h,m,b);$('#guide-preview-dialog').showModal()}catch(err){showToast(err.message,true)}});
  $('#pj-table').addEventListener('click',async e=>{const action=e.target.dataset.pjAction;if(!action)return;const row=pyeongjaeEntries.find(x=>x.id===e.target.closest('[data-pj-id]').dataset.pjId);if(!row)return;if(action==='edit')return pjOpen(row);if(action==='open')return window.open('/pyeongjae-entry?id='+encodeURIComponent(row.id),'_blank','noopener');if(action==='trash'&&!confirm(`“${row.title}”을 휴지통으로 옮길까요?`))return;if(action==='delete'&&!confirm(`“${row.title}”을 완전히 삭제합니다. 복원할 수 없습니다. 계속할까요?`))return;let result;if(action==='toggle')result=await db.from('pyeongjae_entries').update({status:row.status==='published'?'draft':'published',published_at:row.status==='published'?row.published_at:(row.published_at||new Date().toISOString())}).eq('id',row.id);if(action==='trash')result=await db.from('pyeongjae_entries').update({status:'trashed',deleted_at:new Date().toISOString()}).eq('id',row.id);if(action==='restore')result=await db.from('pyeongjae_entries').update({status:'draft',deleted_at:null}).eq('id',row.id);if(action==='delete')result=await db.from('pyeongjae_entries').delete().eq('id',row.id);if(result?.error)showToast(result.error.message,true);else{await loadPyeongjae();showToast('자료 상태를 변경했습니다.')}});  resetCategory();
  initialize();
})();
