(() => {
  ...
  function workCard(row){
    ...
    return `<article class="archive-card works-list-item"><a class="works-list-row" href="${url}" aria-label="${esc(workTitle)} 읽기"><div class="works-list-copy"><span class="works-list-kicker">${esc(workType)}</span><h2>${esc(workTitle)}</h2><p class="works-list-summary">${esc(summary)}</p><div class="works-list-meta"><time datetime="${esc(row.published_at||row.updated_at||'')}">${date(row.published_at||row.updated_at||row.created_at)}</time>${tags.length?`<span class="works-list-tags">${tags.map(tag=>`#${esc(String(tag).replace(/^#+/,''))}`).join(' ')}</span>`:''}</div></div><span class="works-list-thumb">${image?`<img src="${esc(image)}" alt="${esc(row.image_alt||`${workTitle} 대표 이미지`)}" loading="lazy">`:`<span class="works-list-placeholder" aria-hidden="true">언어와 뇌</span>`}</span></a></article>`;
  }
  function card(row){
